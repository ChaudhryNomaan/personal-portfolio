      ...
      <AnimatePresence mode="wait">
        {isOpen && (
          <div className="fixed inset-0 flex items-center justify-center p-4" style={{ zIndex: 999999 }}>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative w-full max-w-md bg-[#0a0a0a] border border-zinc-800 p-10 rounded-[2.5rem] shadow-2xl text-center overflow-hidden" onClick={(e) => e.stopPropagation()}>
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] text-white pt-20 pb-40 px-6 overflow-hidden">
      ...
      <ModalShell isOpen={showSuccess} onClose={() => setShowSuccess(false)}>
        ...
        <button onClick={() => setShowSuccess(false)} className="w-full bg-white hover:bg-amber-500 text-black font-black py-6 rounded-2xl text-[11px] uppercase tracking-[0.4em] transition-all">Confirm Changes</button>
      </ModalShell>

      <div className="max-w-5xl mx-auto relative">
        <Link href="/admin" className="inline-flex items-center gap-2 text-[10px] uppercase tracking-[0.4em] text-neutral-500 hover:text-amber-500 transition-colors mb-12 group">
            ...
            <section className="bg-neutral-900/30 backdrop-blur-xl p-10 rounded-[3rem] border border-white/5 space-y-8">
              ...
              <div className="space-y-6">
                <div className="space-y-3">
                  ...
                  <input name="copyright" type="text" defaultValue={footerData?.copyright || ""} className="w-full bg-white/[0.03] border-b border-white/10 p-5 rounded-2xl outline-none focus:border-amber-500 transition-all text-sm font-light text-white" />
                </div>

                <div className="space-y-3">
                  <label className="text-[9px] uppercase font-bold text-neutral-500 tracking-widest ml-2">Footer Narrative (Bio)</label>
                  <textarea name="narrative" defaultValue={footerData?.narrative || ""} className="w-full bg-white/[0.03] border border-white/5 p-6 rounded-[2.5rem] outline-none focus:border-amber-500 transition-all text-sm font-light min-h-[120px] text-white" />
                </div>
              </div>
            </section>
          ...
          <section className="bg-neutral-900/30 backdrop-blur-xl p-10 rounded-[4rem] border border-white/5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
               <div className="space-y-3">
                  ...
                  <input name="email" defaultValue={footerData?.email || ""} className="w-full bg-white/[0.03] border-b border-white/10 p-4 rounded-xl outline-none focus:border-amber-500 text-sm font-light text-white" />
               </div>
               <div className="space-y-3">
                  <label className="text-[9px] uppercase font-bold text-neutral-500 tracking-widest ml-2">Location</label>
                  <input name="location" defaultValue={footerData?.location || ""} className="w-full bg-white/[0.03] border-b border-white/10 p-4 rounded-xl outline-none focus:border-amber-500 text-sm font-light text-white" />
               </div>
               <div className="space-y-3">
                  <label className="text-[9px] uppercase font-bold text-neutral-500 tracking-widest ml-2">Status</label>
                  <input name="availability" defaultValue={footerData?.availability || ""} className="w-full bg-white/[0.03] border-b border-white/10 p-4 rounded-xl outline-none focus:border-amber-500 text-sm font-light text-white" />
               </div>
