...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // Dynamic Brand State
  const [brand, setBrand] = useState({ name: "STUDIO", logo_url: "" });

  const fetchBrand = useCallback(async () => {
    const { data } = await supabase.from('site_config').select('content').eq('id', 'hero_content').single();
    if (data?.content) {
      setBrand({ 
        name: data.content.studio_name || "STUDIO", 
    ...
    <div className="flex flex-col h-full bg-[#0a0a0a] border-r border-white/5 select-none overflow-y-auto scrollbar-hide">
      ...
      <div className="p-6 lg:p-10 mb-4 flex justify-between items-center">
        <Link href="/admin/dashboard" className="flex items-center gap-4 group">
          <div className="w-10 h-10 lg:w-12 lg:h-12 bg-white rounded-2xl flex items-center justify-center overflow-hidden border border-white/10 group-hover:border-amber-500/50 transition-all duration-500">
            {brand.logo_url ? (
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[7px] text-amber-500/60 uppercase tracking-[0.3em] font-mono mt-1">Admin Engine v2.4</span>
          </div>
        </Link>
      ...
      <nav className="flex-1 px-4 lg:px-6 space-y-1 lg:space-y-2">
        {NAV.map((item) => {
          ...
          return (
            ...
            >
              {active && <motion.div layoutId="navActive" className="absolute left-0 w-[2px] h-5 lg:h-6 bg-amber-500 rounded-full" />}
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#e5e5e5] lg:cursor-none selection:bg-amber-500/30">
      ...
      <aside className={`fixed top-0 left-0 h-full w-[280px] z-[70] transition-transform duration-500 lg:hidden bg-[#0a0a0a] ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}><SidebarContent /></aside>

      <div className="flex-1 lg:ml-72 flex flex-col min-h-screen">
        <header className={`sticky top-0 z-[55] px-4 sm:px-12 py-4 lg:py-6 flex justify-between items-center transition-all duration-500 ${scrolled ? 'bg-[#0a0a0a]/80 backdrop-blur-xl border-b border-white/5' : 'bg-transparent'}`}>
          ...
          <div className="flex items-center gap-3 lg:gap-6">
            <div className="relative hidden md:block">
               ...
               <input type="text" placeholder="System Search..." className="bg-white/5 border border-white/10 rounded-full py-2.5 pl-10 pr-6 text-[9px] font-bold uppercase tracking-widest outline-none focus:border-amber-500/50 transition-all w-64 text-white" />
            </div>
