          ...
          <div className="flex flex-col md:flex-row justify-between items-end border-b border-white/5 pb-10 gap-8">
            <div className="max-w-2xl">
              <div className="flex items-center gap-3 mb-6">
                <motion.div initial={{ width: 0 }} animate={{ width: 30 }} className="h-px bg-amber-500" />
                <span className="text-amber-500 uppercase tracking-[0.8em] text-[8px] font-bold block">Portfolio</span>
              </div>
          ...
          <nav className="flex gap-8 md:gap-10 pt-8 overflow-x-auto no-scrollbar">
            {categories.map((cat) => (
              ...
              >
                {cat}
                {activeFilter === cat && (
                  <motion.div layoutId="activeFilter" className="absolute bottom-0 left-0 w-full h-[1px] bg-amber-500" />
                )}
              </button>
            ))}
          </nav>
